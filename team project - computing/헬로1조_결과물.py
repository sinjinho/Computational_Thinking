...
days = ["월", "화", "수", "목", "금", "토", "일"] # 요일 문제열 리스트
todo_values = [[] for _ in range(7)]             # 요일별 실제 할 일 리스트(이중임) 
default_values = [[] for _ in range(7)]          # 요일별 기본값 리스트(이중임)
value_frames = []                                # 기본값 항목들을 담는 프레임 리스트
check_status = [[] for _ in range(7)]            # 할 일의 체크 여부 상태 - bool값
checkbuttons = [[] for _ in range(7)]            # 체크박스 위젯 저장용 리스트
int_vars = [[] for _ in range(7)]                # 체크박스 상태와 연결된 IntVar 리스트
entry_vars_main = []                             # 메인 화면 입력창용
entry_vars_default = []                          # 기본값 설정 화면 입력창용
entry_widgets = []                               # 실제 입력창(Entry 위젯) 리스트
day_task_frames = []                             # 요일별 할 일 항목을 표시하는 프레임 리스트

graph_canvas = None                              # 그래프 위젯을 저장할 변수

box_width = 130                                  # 요일 박스의 너비/높이 (UI 크기 설정용)
box_height = 400
wrap_px = box_width - 50                         # 텍스트 줄바꿈 기준 픽셀 수
...
# 시간중복체크
# parameter: 요일 인덱스, 추가할 시간 문자열, 기준이 될 할 일 목록
# 시간 중복 시 true반환
...
# 입력값을 기준으로 값을 리스트에 추가
# parameter: 요일 인덱스, 요일별 입력창의 텍스트 변수 리스트, 추가 대상 리스트, 체크 상태 저장 리스트, UI를 다시 그리는 함수, 그래프를 다시 그리는 함수
...
# 메인 할 일 + 버튼에서 호출되는 함수 -> 할 일 리스트에 추가
...
# 값 삭제를 처리하는 함수
# parameter: 요일 인덱스, 삭제할 항목의 인덱스, 삭제 대상 리스트, 체크 상태 저장 리스트, UI를 다시 그리는 함수, 실행할 UI 함수 목록
def delete_value(day_idx,item_idx,target_values,check_status_list=None,refresh_ui_fn=None,extra_ui_hooks=None):
    # 항목 제거
...
# 기본값 삭제 처리            
# parameter: 삭제할 값의 위치(이중 리스트라서 2개의 값)
...
# 체크박스를 포함한 할 일을 생성
# parameter: 각 인덱스와 텍스트 내용, 부모 프레임
...
# 날씨 추천 문구 표시 + 주간 초기화 버튼
def display_weather_recommendation(parent_frame):
    # 프레임 안에서 좌우로 배치
...
# 주간 초기화 버튼 구현 - 주간 할 일 데이터를 초기화하고 기본값으로 재설정
